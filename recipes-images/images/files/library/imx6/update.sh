...
case $MODTYPE_DETECT in
	0)	# auto detect MODTYPE from rootfs directory
		if [ -f rootfs/etc/issue ] ; then
			...
			if [ "$CNT" -ge 1 ] ; then
				echo "Colibri iMX6 rainbow rootfs detected"
				MODTYPE=colibri-imx6-rainbow
			fi
		fi
		if [ -e $MODTYPE ] ; then
		...
		;;
	1)	MODTYPE=colibri-imx6-rainbow
...
case "$MODTYPE" in
	"colibri-imx6-rainbow")
		...
		OUT_DIR="$OUT_DIR/colibri_imx6_rainbow"
		;;
