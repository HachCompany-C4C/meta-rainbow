...
case $MODTYPE_DETECT in
	0)	# auto detect MODTYPE from rootfs directory
		if [ -f rootfs/etc/issue ] ; then
			CNT=`grep -ic "iMX7" rootfs/etc/issue || true`
			if [ "$CNT" -ge 1 ] ; then
				echo "Colibri iMX7 rainbow rootfs detected"
				MODTYPE=colibri-imx7-rainbow
			fi
		fi
		if [ -e $MODTYPE ] ; then
		...
		;;
	2)	MODTYPE=colibri-imx7-rainbow
		echo "Colibri iMX6 rainbow rootfs specified"
		;;
...
case "$MODTYPE" in
    "colibri-imx7-rainbow")
		...
		OUT_DIR="$OUT_DIR/colibri_imx7_rainbow"
		;;
...
ROOTFSVERSION=`grep -i imx7 rootfs/etc/issue || echo "Version Unknown"`
...
#copy to $OUT_DIR
sudo cp ${BINARIES}/${U_BOOT_BINARY}  ${BINARIES}/${KERNEL_IMAGETYPE} ${BINARIES}/mbr.bin \
